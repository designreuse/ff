	...
	public Predicate toPredicate(Root<User> root, CriteriaQuery<?> query, CriteriaBuilder builder) {
		switch (criteria.getOperation()) {
			...
			case ENDS_WITH:
				return builder.like(root.<String> get(criteria.getKey()), "%" + criteria.getValue());
			case CONTAINS:
				if (criteria.getKey().equals("company.name")) {
					return builder.like(root.<Company> get("company").<String>get("name"), "%" + criteria.getValue() + "%");
				} else if (criteria.getKey().equals("company.code")) {
					return builder.like(root.<Company> get("company").<String>get("code"), "%" + criteria.getValue() + "%");
				}
				return builder.like(root.<String> get(criteria.getKey()), "%" + criteria.getValue() + "%");
			case BETWEEN:
