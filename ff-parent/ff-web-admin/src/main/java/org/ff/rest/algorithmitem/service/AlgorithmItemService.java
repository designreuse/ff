package org.ff.rest.algorithmitem.service;
...
public class AlgorithmItemService extends BaseService {
	...
	private AlgorithmItemSpecification createSpecification(UiGridFilterResource resource) {
		...
		} else if (resource.getName().equalsIgnoreCase("operator.value")) {
			if (EnumUtils.isValidEnum(Operator.class, resource.getTerm().toUpperCase())) {
				return new AlgorithmItemSpecification(new SearchCriteria("operator", SearchOperation.EQUALITY, Operator.valueOf(resource.getTerm().toUpperCase())));
			} else {
				return null;
			}
