...
public class EmailController extends BaseController {
	...
	public void send(Principal principal, @RequestBody SendEmailResource resource) {
		EtmPoint point = etmService.createPoint(getClass().getSimpleName() + ".send");
		try {
			emailService.send(resource);
		} finally {
			etmService.collect(point);
		}
	}
