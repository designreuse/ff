angular.module('FundFinder')
	.controller('ActivitiesOverviewController', ActivitiesOverviewController)
	.controller('ActivitiesEditController', ActivitiesEditController);

// ========================================================================
//	OVERVIEW CONTROLLER
// ========================================================================
function ActivitiesOverviewController($rootScope, $scope, $state, $log, $timeout, $filter, uiGridConstants, Upload, ActivitiesService) {
	...
	$scope.getPage = function(page, size) {
		...
		ActivitiesService.getPage(uiGridResource)
	...
	$scope.editEntity = function (entity) {
		$state.go('settings.activities_edit', { 'id' : entity.id });
	}
	
	$scope.importData = function(files) {
		if (files && files.length) {
			Upload.upload({
		        url: '/api/v1/activities/import',
		        method: 'POST',
		        file: files[0]
		    }).success(function (data, status, headers, config) {
...
//	EDIT CONTROLLER
// ========================================================================
function ActivitiesEditController($rootScope, $scope, $state, $stateParams, $log, $timeout, $filter, uiGridConstants, ActivitiesService) {
	var $translate = $filter('translate');
	
	$scope.getEntity = function(id) {
		ActivitiesService.getEntity(id)
			.success(function(data, status) {
				if (status == 200) {
					$scope.entity = data;
				} else {
	...
	$scope.saveEntity = function() {
		ActivitiesService.saveEntity($scope.entity)
			.success(function(data, status, headers, config) {
				if (status == 200) {
					toastr.success($translate('ACTION_SAVE_SUCCESS_MESSAGE'));
					$state.go('settings.activities_edit', { 'id' : data.id });
				} else {
	...
	$scope.back = function() {
		$state.go('settings.activities_overview');
	};
