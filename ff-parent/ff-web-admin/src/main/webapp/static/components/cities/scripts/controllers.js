...
//	OVERVIEW CONTROLLER
// ========================================================================
function CitiesOverviewController($rootScope, $scope, $state, $log, $timeout, $filter, uiGridConstants, CitiesService) {
	...
	$scope.getPage = function(page, size) {
		...
		CitiesService.getPage(uiGridResource)
	...
	$scope.addEntity = function (entity) {
		$state.go('settings.cities_edit', { 'id' : 0 });
	}
	...
	$scope.deleteEntity = function (entity) {
		BootstrapDialog.show({
            ...
            title: $translate('DIALOG_DELETE_HEADER', { entity: $lowercase($translate('ENTITY_CITY')) }),
            message: $translate('DIALOG_DELETE_MESSAGE', { entity: $lowercase($translate('ENTITY_CITY')) }),
            buttons: [
				{
				...
				},
            	{
	                ...
	                action: function(dialog) {
	                	CitiesService.deleteEntity(entity.id)
		    				.success(function(data, status) {
		    					if (status == 200) {
		    						toastr.success($translate('ACTION_DELETE_SUCCESS_MESSAGE', { entity: $translate('ENTITY_CITY') }));
		    						$scope.getPage($scope.gridApi.pagination.getPage(), $scope.gridOptions.paginationPageSize);
		    					} else {
		    						toastr.error($translate('ACTION_DELETE_FAILURE_MESSAGE', { entity: $translate('ENTITY_CITY') }));
		    					}
		    				})
...
//	EDIT CONTROLLER
// ========================================================================
function CitiesEditController($rootScope, $scope, $state, $stateParams, $log, $timeout, $filter, uiGridConstants, CitiesService, CountiesService) {
	var $translate = $filter('translate');
	...
	$scope.saveEntity = function() {
		CitiesService.saveEntity($scope.entity)
			.success(function(data, status, headers, config) {
				if (status == 200) {
					toastr.success($translate('ACTION_SAVE_SUCCESS_MESSAGE'));
					$state.go('settings.cities_edit', { 'id' : data.id });
				} else {
