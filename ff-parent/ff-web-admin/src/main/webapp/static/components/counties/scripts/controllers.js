angular.module('FundFinder')
	.controller('CountiesOverviewController', CountiesOverviewController)
	.controller('CountiesEditController', CountiesEditController);

// ========================================================================
//	OVERVIEW CONTROLLER
// ========================================================================
function CountiesOverviewController($rootScope, $scope, $state, $log, $timeout, $filter, uiGridConstants, CountiesService) {
	...
	$scope.getPage = function(page, size) {
		...
		CountiesService.getPage(uiGridResource)
	...
	$scope.addEntity = function (entity) {
		$state.go('settings.counties_edit', { 'id' : 0 });
	}
	
	$scope.editEntity = function (entity) {
		$state.go('settings.counties_edit', { 'id' : entity.id });
	}
	
	$scope.deleteEntity = function (entity) {
		BootstrapDialog.show({
			type: BootstrapDialog.TYPE_DEFAULT,
            title: $translate('DIALOG_DELETE_HEADER', { entity: $lowercase($translate('ENTITY_COUNTY')) }),
            message: $translate('DIALOG_DELETE_MESSAGE', { entity: $lowercase($translate('ENTITY_COUNTY')) }),
            buttons: [
				{
				...
				},
            	{
	                ...
	                action: function(dialog) {
	                	CountiesService.deleteEntity(entity.id)
		    				.success(function(data, status) {
		    					if (status == 200) {
		    						toastr.success($translate('ACTION_DELETE_SUCCESS_MESSAGE', { entity: $translate('ENTITY_COUNTY') }));
		    						$scope.getPage($scope.gridApi.pagination.getPage(), $scope.gridOptions.paginationPageSize);
		    					} else {
		    						toastr.error($translate('ACTION_DELETE_FAILURE_MESSAGE', { entity: $translate('ENTITY_COUNTY') }));
		    					}
		    				})
...
//	EDIT CONTROLLER
// ========================================================================
function CountiesEditController($rootScope, $scope, $state, $stateParams, $log, $timeout, $filter, uiGridConstants, CountiesService) {
	var $translate = $filter('translate');
	
	$scope.getEntity = function(id) {
		CountiesService.getEntity(id)
			.success(function(data, status) {
				if (status == 200) {
					$scope.entity = data;
				} else {
	...
	$scope.saveEntity = function() {
		CountiesService.saveEntity($scope.entity)
			.success(function(data, status, headers, config) {
				if (status == 200) {
					toastr.success($translate('ACTION_SAVE_SUCCESS_MESSAGE'));
					$state.go('settings.counties_edit', { 'id' : data.id });
				} else {
	...
	$scope.back = function() {
		$state.go('settings.counties_overview');
	};
