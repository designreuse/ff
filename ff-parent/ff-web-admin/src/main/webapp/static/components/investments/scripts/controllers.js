angular.module('FundFinder')
	.controller('InvestmentsOverviewController', InvestmentsOverviewController)
	.controller('InvestmentsDetailsController', InvestmentsDetailsController)
	.controller('InvestmentsEditController', InvestmentsEditController);

// ========================================================================
//	OVERVIEW CONTROLLER
// ========================================================================
function InvestmentsOverviewController($rootScope, $scope, $state, $log, $timeout, $filter, uiGridConstants, InvestmentsService) {
	...
	$scope.getPage = function(page, size) {
		...
		InvestmentsService.getPage(uiGridResource)
	...
	$scope.showEntity = function (entity) {
		$state.go('investments.details', { 'id' : entity.id, 'showEditButton' : true });
	}
	...
	$scope.editEntity = function (entity) {
		$state.go('investments.edit', { 'id' : entity.id });
	}
	
	$scope.activateEntity = function(entity) {
		InvestmentsService.activateEntity(entity.id)
			.success(function(data, status) {
				if (status == 200) {
					toastr.success($translate('ACTION_ACTIVATE_SUCCESS_MESSAGE', { entity: $translate('ENTITY_INVESTMENT') }));
					$scope.getPage($scope.gridApi.pagination.getPage(), $scope.gridOptions.paginationPageSize);
				} else {
					toastr.error($translate('ACTION_ACTIVATE_FAILURE_MESSAGE', { entity: $translate('ENTITY_INVESTMENT') }));
				}
			})
	...
	$scope.deactivateEntity = function(entity) {
		InvestmentsService.deactivateEntity(entity.id)
			.success(function(data, status) {
				if (status == 200) {
					toastr.success($translate('ACTION_DEACTIVATE_SUCCESS_MESSAGE', { entity: $translate('ENTITY_INVESTMENT') }));
					$scope.getPage($scope.gridApi.pagination.getPage(), $scope.gridOptions.paginationPageSize);
				} else {
					toastr.error($translate('ACTION_DEACTIVATE_FAILURE_MESSAGE', { entity: $translate('ENTITY_INVESTMENT') }));
				}
			})
			.error(function(data, status) {
				toastr.error($translate('ACTION_DEACTIVATE_FAILURE_MESSAGE', { entity: $translate('ENTITY_INVESTMENT') }));
	...
	$scope.deleteEntity = function (entity) {
		BootstrapDialog.show({
            ...
            title: $translate('DIALOG_DELETE_HEADER', { entity: $lowercase($translate('ENTITY_INVESTMENT')) }),
            message: $translate('DIALOG_DELETE_MESSAGE', { entity: $lowercase($translate('ENTITY_INVESTMENT')) }),
            buttons: [
				{
				...
				},
            	{
	                ...
	                action: function(dialog) {
	                	InvestmentsService.deleteEntity(entity.id)
		    				.success(function(data, status) {
		    					if (status == 200) {
		    						toastr.success($translate('ACTION_DELETE_SUCCESS_MESSAGE', { entity: $translate('ENTITY_INVESTMENT') }));
		    						$scope.getPage($scope.gridApi.pagination.getPage(), $scope.gridOptions.paginationPageSize);
		    					} else {
		    						toastr.error($translate('ACTION_DELETE_FAILURE_MESSAGE', { entity: $translate('ENTITY_INVESTMENT') }));
		    					}
		    				})
		    				.error(function(data, status) {
		    					toastr.error($translate('ACTION_DELETE_FAILURE_MESSAGE', { entity: $translate('ENTITY_INVESTMENT') }));
	...
	$timeout(function() {
		...
		// watch 'cntInvestments' variable, so if it changes we can refresh grid
		$scope.$watch('cntInvestments', function(newValue, oldValue) {
...
//	DETAILS CONTROLLER
// ========================================================================
function InvestmentsDetailsController($rootScope, $scope, $state, $stateParams, $log, $timeout, $filter, $sce, uiGridConstants, InvestmentsService) {
	var $translate = $filter('translate');
	...
	$scope.editEntity = function() {
		$state.go('investments.edit', { 'id' : $stateParams.id });
	}
	
	$scope.back = function() {
		if ($rootScope.previousState) {
			if ($rootScope.currentState == 'investments.details') {
				if ($rootScope.previousState == 'users.details') {
					$state.go($rootScope.previousState, $rootScope.previousStateParams);
				} else {
					$state.go('investments.overview');	
				}
			} else if ($rootScope.previousState == 'investments.edit' && $rootScope.previousStateParams.id == '0') {
				$state.go('investments.overview');
			} else {
				$state.go($rootScope.previousState, $rootScope.previousStateParams);
			}
	...
	// initial load
	InvestmentsService.getEntity($stateParams.id)
		.success(function(data, status) {
			if (status == 200) {
				$scope.entity = data;
			} else {
...
//	EDIT CONTROLLER
// ========================================================================
function InvestmentsEditController($rootScope, $scope, $state, $stateParams, $log, $timeout, $filter, uiGridConstants, InvestmentsService) {
	var $translate = $filter('translate');
	...
	$scope.getEntity = function(id) {
		InvestmentsService.getEntity(id)
			.success(function(data, status) {
				if (status == 200) {
					$scope.entity = data;
				} else {
	...
	$scope.saveEntity = function() {
		InvestmentsService.saveEntity($scope.entity)
			.success(function(data, status, headers, config) {
				if (status == 200) {
					toastr.success($translate('ACTION_SAVE_SUCCESS_MESSAGE'));
					$state.go('investments.edit', { 'id' : data.id });
				} else {
