angular.module('FundFinder')
	.controller('NkdsOverviewController', NkdsOverviewController)
	.controller('NkdsEditController', NkdsEditController);

// ========================================================================
//	OVERVIEW CONTROLLER
// ========================================================================
function NkdsOverviewController($rootScope, $scope, $state, $log, $timeout, $filter, uiGridConstants, NkdsService) {
	...
	$scope.gridOptions = {
			...
			columnDefs: [
				...
				{
					displayName: $translate('COLUMN_SECTOR'),
					field: 'sector',
					type: 'string',
					cellTooltip: false, 
					enableSorting: true,
	...
	$scope.getPage = function(page, size) {
		...
		NkdsService.getPage(uiGridResource)
	...
	$scope.addEntity = function (entity) {
		$state.go('settings.nkds_edit', { 'id' : 0 });
	}
	
	$scope.editEntity = function (entity) {
		$state.go('settings.nkds_edit', { 'id' : entity.id });
	}
	
	$scope.deleteEntity = function (entity) {
		BootstrapDialog.show({
			type: BootstrapDialog.TYPE_DEFAULT,
            title: $translate('DIALOG_DELETE_HEADER', { entity: $translate('ENTITY_NKD') }),
            message: $translate('DIALOG_DELETE_MESSAGE', { entity: $translate('ENTITY_NKD') }),
            buttons: [
				{
				...
				},
            	{
	                ...
	                action: function(dialog) {
	                	NkdsService.deleteEntity(entity.id)
		    				.success(function(data, status) {
		    					if (status == 200) {
		    						toastr.success($translate('ACTION_DELETE_SUCCESS_MESSAGE', { entity: $translate('ENTITY_NKD') }));
		    						$scope.getPage($scope.gridApi.pagination.getPage(), $scope.gridOptions.paginationPageSize);
		    					} else {
		    						toastr.error($translate('ACTION_DELETE_FAILURE_MESSAGE', { entity: $translate('ENTITY_NKD') }));
		    					}
		    				})
...
//	EDIT CONTROLLER
// ========================================================================
function NkdsEditController($rootScope, $scope, $state, $stateParams, $log, $timeout, $filter, uiGridConstants, NkdsService) {
	var $translate = $filter('translate');
	
	$scope.getEntity = function(id) {
		NkdsService.getEntity(id)
			.success(function(data, status) {
				if (status == 200) {
					$scope.entity = data;
				} else {
	...
	$scope.saveEntity = function() {
		NkdsService.saveEntity($scope.entity)
			.success(function(data, status, headers, config) {
				if (status == 200) {
					toastr.success($translate('ACTION_SAVE_SUCCESS_MESSAGE'));
					$state.go('settings.nkds_edit', { 'id' : data.id });
				} else {
	...
	$scope.back = function() {
		$state.go('settings.nkds_overview');
	};
