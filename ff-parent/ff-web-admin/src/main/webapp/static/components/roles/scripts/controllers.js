angular.module('FundFinder')
	.controller('RolesOverviewController', RolesOverviewController)
	.controller('RolesEditController', RolesEditController);

// ========================================================================
//	OVERVIEW CONTROLLER
// ========================================================================
function RolesOverviewController($rootScope, $scope, $state, $log, $timeout, $filter, uiGridConstants, RolesService) {
	...
	$scope.getPage = function(page, size) {
		...
		RolesService.getPage(uiGridResource)
	...
	$scope.editEntity = function (entity) {
		$state.go('settings.roles_edit', { 'id' : entity.id });
	}
	
	$scope.deleteEntity = function (entity) {
		BootstrapDialog.show({
			type: BootstrapDialog.TYPE_DEFAULT,
            title: $translate('DIALOG_DELETE_HEADER', { entity: $lowercase($translate('ENTITY_ROLE')) }),
            message: $translate('DIALOG_DELETE_MESSAGE', { entity: $lowercase($translate('ENTITY_ROLE')) }),
            buttons: [
				{
				...
				},
            	{
	                ...
	                action: function(dialog) {
	                	RolesService.deleteEntity(entity.id)
		    				.success(function(data, status) {
		    					if (status == 200) {
		    						toastr.success($translate('ACTION_DELETE_SUCCESS_MESSAGE', { entity: $translate('ENTITY_ROLE') }));
		    						$scope.getPage($scope.gridApi.pagination.getPage(), $scope.gridOptions.paginationPageSize);
		    					} else {
		    						toastr.error($translate('ACTION_DELETE_FAILURE_MESSAGE', { entity: $translate('ENTITY_ROLE') }));
		    					}
		    				})
...
//	EDIT CONTROLLER
// ========================================================================
function RolesEditController($rootScope, $scope, $state, $stateParams, $log, $timeout, $filter, uiGridConstants, RolesService, PermissionsService) {
	var $translate = $filter('translate');
	
	$scope.getEntity = function(id) {
		RolesService.getEntity(id)
			.success(function(data, status) {
				if (status == 200) {
					$scope.entity = data;
				} else {
	...
	$scope.saveEntity = function() {
		RolesService.saveEntity($scope.entity)
			.success(function(data, status, headers, config) {
				if (status == 200) {
					toastr.success($translate('ACTION_SAVE_SUCCESS_MESSAGE'));
					$state.go('settings.roles_edit', { 'id' : data.id });
				} else {
	...
	$scope.getPermissions = function() {
		PermissionsService.getEntities()
			.success(function(data, status) {
				if (status == 200) {
	...
	$scope.back = function() {
		$state.go('settings.roles_overview');
	};
