angular.module('FundFinder')
	.controller('Subdivisions1OverviewController', Subdivisions1OverviewController)
	.controller('Subdivisions1EditController', Subdivisions1EditController);

// ========================================================================
//	OVERVIEW CONTROLLER
// ========================================================================
function Subdivisions1OverviewController($rootScope, $scope, $state, $log, $timeout, $filter, uiGridConstants, Subdivisions1Service) {
	...
	$scope.getPage = function(page, size) {
		...
		Subdivisions1Service.getPage(uiGridResource)
	...
	$scope.editEntity = function (entity) {
		$state.go('settings.subdivisions1_edit', { 'id' : entity.id });
	}
...
//	EDIT CONTROLLER
// ========================================================================
function Subdivisions1EditController($rootScope, $scope, $state, $stateParams, $log, $timeout, $filter, uiGridConstants, Subdivisions1Service) {
	var $translate = $filter('translate');
	
	$scope.getEntity = function(id) {
		Subdivisions1Service.getEntity(id)
			.success(function(data, status) {
				if (status == 200) {
					$scope.entity = data;
				} else {
	...
	$scope.saveEntity = function() {
		Subdivisions1Service.saveEntity($scope.entity)
			.success(function(data, status, headers, config) {
				if (status == 200) {
					toastr.success($translate('ACTION_SAVE_SUCCESS_MESSAGE'));
					$state.go('settings.subdivisions1_edit', { 'id' : data.id });
				} else {
	...
	$scope.back = function() {
		$state.go('settings.subdivisions1_overview');
	};
