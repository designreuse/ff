angular.module('FundFinder')
	.controller('UserGroupsOverviewController', UserGroupsOverviewController)
	.controller('UserGroupsEditController', UserGroupsEditController);

// ========================================================================
//	OVERVIEW CONTROLLER
// ========================================================================
function UserGroupsOverviewController($rootScope, $scope, $state, $log, $timeout, $filter, uiGridConstants, UserGroupsService) {
	...
	$scope.getPage = function(page, size) {
		...
		UserGroupsService.getPage(uiGridResource)
	...
	$scope.editEntity = function (entity) {
		$state.go('settings.usergroups_edit', { 'id' : entity.id });
	}
	
	$scope.activateEntity = function(entity) {
		UserGroupsService.activateEntity(entity.id)
			.success(function(data, status) {
				if (status == 200) {
					toastr.success($translate('ACTION_ACTIVATE_SUCCESS_MESSAGE', { entity: $translate('ENTITY_USER_GROUP') }));
					$scope.getPage($scope.gridApi.pagination.getPage(), $scope.gridOptions.paginationPageSize);
				} else {
					toastr.error($translate('ACTION_ACTIVATE_FAILURE_MESSAGE', { entity: $translate('ENTITY_USER_GROUP') }));
				}
			})
	...
	$scope.deactivateEntity = function(entity) {
		UserGroupsService.deactivateEntity(entity.id)
			.success(function(data, status) {
				if (status == 200) {
					toastr.success($translate('ACTION_DEACTIVATE_SUCCESS_MESSAGE', { entity: $translate('ENTITY_USER_GROUP') }));
					$scope.getPage($scope.gridApi.pagination.getPage(), $scope.gridOptions.paginationPageSize);
				} else {
					toastr.error($translate('ACTION_DEACTIVATE_FAILURE_MESSAGE', { entity: $translate('ENTITY_USER_GROUP') }));
				}
			})
			.error(function(data, status) {
				toastr.error($translate('ACTION_DEACTIVATE_FAILURE_MESSAGE', { entity: $translate('ENTITY_USER_GROUP') }));
	...
	$scope.deleteEntity = function (entity) {
		BootstrapDialog.show({
            ...
            title: $translate('DIALOG_DELETE_HEADER', { entity: $lowercase($translate('ENTITY_USER_GROUP')) }),
            message: $translate('DIALOG_DELETE_MESSAGE', { entity: $lowercase($translate('ENTITY_USER_GROUP')) }),
            buttons: [
				{
				...
				},
            	{
	                ...
	                action: function(dialog) {
	                	UserGroupsService.deleteEntity(entity.id)
		    				.success(function(data, status) {
		    					if (status == 200) {
		    						toastr.success($translate('ACTION_DELETE_SUCCESS_MESSAGE', { entity: $translate('ENTITY_USER_GROUP') }));
		    						$scope.getPage($scope.gridApi.pagination.getPage(), $scope.gridOptions.paginationPageSize);
		    					} else {
		    						toastr.error($translate('ACTION_DELETE_FAILURE_MESSAGE', { entity: $translate('ENTITY_USER_GROUP') }));
		    					}
		    				})
		    				.error(function(data, status) {
		    					toastr.error($translate('ACTION_DELETE_FAILURE_MESSAGE', { entity: $translate('ENTITY_USER_GROUP') }));
	...
	$timeout(function() {
		...
		// watch 'cntUserGroup' variable, so if it changes we can refresh grid
		$scope.$watch('cntUserGroup', function(newValue, oldValue) {
...
//	EDIT CONTROLLER
// ========================================================================
function UserGroupsEditController($rootScope, $scope, $state, $stateParams, $log, $timeout, $filter, uiGridConstants, UserGroupsService, UsersService) {
	var $translate = $filter('translate');
	
	$scope.getEntity = function(id) {
		UserGroupsService.getEntity(id)
			.success(function(data, status) {
				if (status == 200) {
					$scope.entity = data;
				} else {
	...
	$scope.saveEntity = function() {
		UserGroupsService.saveEntity($scope.entity)
			.success(function(data, status, headers, config) {
				if (status == 200) {
					toastr.success($translate('ACTION_SAVE_SUCCESS_MESSAGE'));
					$state.go('settings.usergroups_edit', { 'id' : data.id });
				} else {
	...
	$scope.back = function() {
		$state.go('settings.usergroups_overview');
	};
