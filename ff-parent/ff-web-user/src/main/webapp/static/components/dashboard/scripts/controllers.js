...
//	OVERVIEW
// ========================================================================
function DashboardController($rootScope, $scope, $state, $log, $timeout, $filter, SessionStorage, CommonService, DashboardService, ArticlesService, ImagesService, TendersService) {
			...
			.success(function(data, status) {
				if (status == 200) {
					...
					if ($rootScope.principal.demoUser) {
						...
						$scope.data.cntProjects = projects.length;
						
						var demoResource = { "company" : SessionStorage.getSession("company"), "projects" : SessionStorage.getSession("projects") };
						TendersService.findAllDemo(demoResource)
							.success(function(data, status, headers, config) {
								if (status == 200) {
									$scope.data.cntTenders4U = data.length;
								} else {
	...
	$scope.getImage = function(article) {
		ImagesService.getEntity(article.imageId)
			.success(function(data, status) {
				if (status == 200) {
					article.image = data;
				} else {
